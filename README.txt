...
Para rodar esta API é necessário ter instalado Python 3 em sua máquina. Cumprindo esse requisito, abra o prompet de 
comando (se windows) ou terminal (se linux) na pasta chamada 'api' e execute o comando abaixo:

	python api.txt

    e um servidor será inicializado no endereço IP 'http://127.0.0.1:5000' (localhost).

Após, abra o arquivo 'api.txt' e na variavel 'palavras' (sétima linha) adicione as palavras que desejar contar as vogais, entre 
...
This is an API written for a job interview at Blintz startup, in its version 1.0.

This API aims to receive a JSON with strings and return another JSON with the same strings, but as dict with those strings 
    as keys and the number os voels as values.

To run this API you must have installed Python 3 in your machine. This done, open the command prompet (for windows)
    or terminal (for linux) at 'api' directory and execute the following command:
